	...
	def process(self, path_seg, path_original, error_limit):
		...
		for color in colors:
			...
			for boundary_points_ in self.edge_detection(masked_img):
				...
				boundary_points_ = map(lambda x : np.asarray([x[1], x[0]]), boundary_points_)
				boundary_points_ = np.asarray(boundary_points_)
...
# args : matlab_result_image, actual_image, error
