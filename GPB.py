		...
		for row in range(height):
			for col in range(width):
				...
				for i in range(0, 360, 60):
					...
					cv2.ellipse(circle_img_2, (col, row), (5,5), 0, (i+180), (i + 180 + 180), (255,255,255), thickness = -1)
			
					# masked_data_1 = cv2.bitwise_and(im, im, mask=circle_img_1)
					# masked_data_2 = cv2.bitwise_and(im, im, mask=circle_img_2)

					hist_1_channel0 = cv2.calcHist([self.input_image], [0], circle_img_1, [256], [0,256])
					hist_2_channel0 = cv2.calcHist([self.input_image], [0], circle_img_2, [256], [0,256])
