...
class Segmentation():
	...
	def dfs(self, cimg, x, y, points, vis, h, w):
		...
		for k in nexx:
			x_, y_ = x + k[0], y + k[1]
			if x_ < h and x_ >=0 and y_<w and y_>=0 and cimg[x_][y_]==255 and vis[x_][y_]!=True:
				self.dfs(cimg, x_, y_, points, vis, h, w)
	...
	def edge_detection(self, img, color):
		...
		for i in range(len(contours)):
			...
			if area > max_area:
				max_area = area
				req_contour = contours[i]
		
		points = np.reshape(req_contour, (req_contour.shape[0],2) )
		points = map(lambda x : [x[1], x[0]], points)
...
ss.process(sys.argv[1], sys.argv[2], 10**-3)